...
// NOVO: Estrutura para os dados de localização (tabela separada)
typedef struct {
...
typedef struct {
    char nu_seq[15];
    int ano;
    // OTIMIZAÇÃO O(1): Substituímos cod_esc por um ponteiro direto (índice)
...
// NOVO: Busca de Localização por índice (O(1))
...
// FIM DAS FUNÇÕES DE MANIPULAÇÃO DO ARQUIVO DE LOCALIZAÇÃO
...
int importar_participantes_csv(char *nome_csv, const char *nome_bin) {
    ...
    // Variáveis temporárias para dados que não vão para Participante, mas são necessários para Localizacao
    char temp_cod_esc[15];
    char temp_cidade[40];
    char temp_estado[20];

    // Format string adaptada:
    // Captura: 1.NU_SEQ, 2.ANO, 3.COD_ESC (temp), 4.CIDADE (temp), 5.ESTADO (temp), +14 campos (4xCod, 4xNota, 4xResp, Ling, Red) = 19 argumentos
    const char *csv_format_final =
        ...
        "%19[^;];"                // 7- ESTADO (temp)
        "%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];" // Skips
        "%f;%f;%f;%f;"           // 19 a 22 (COD_PROVA...)
        "%f;%f;%f;%f;"           // 23 a 26 (NOTAS CN, CH, LC, MT)
        "%49[^;];%49[^;];%49[^;];%49[^;];" // 27 a 30 (RESPOSTAS CN, CH, LC, MT)
        "%d;"                    // 31 (LING_EST)
        "%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];%*[^;];" // Skips
        "%f";                    // 42 (NOTA_RED)
    ...
    while (fgets(linha, sizeof(linha), fp_csv) != NULL) {
        ...
        int ok = sscanf(
            ...
            p.nu_seq,
            &p.ano,
            temp_cod_esc, // 3. COD_ESC (temp)
            temp_cidade,  // 4. CIDADE (temp)
            temp_estado,  // 5. ESTADO (temp)
            &p.cod_prova_cn,
            &p.cod_prova_ch,
            &p.cod_prova_lc,
            &p.cod_prova_mt,
        ...
        // O número esperado de argumentos lidos é 19
        if (ok != 19) {
             printf("Linha ignorada (formato inesperado). Lidos = %d. Esperado = 19\n", ok);
             continue;
        }

        // --- 1. PROCESSAMENTO DE LOCALIZAÇÃO (OTIMIZAÇÃO O(1)) ---

        // Busca se o código da escola já existe
        int indice_loc = buscar_indice_localizacao_por_cod_esc(fp_loc, &header_loc, temp_cod_esc);

        if (indice_loc == -1) {
            // Se NOVO: Salva a nova localização e captura o índice de retorno
        ...
        // Associa o índice ao Participante
        p.indice_localizacao = indice_loc;


        // --- 2. INSERIR PARTICIPANTE E ÍNDICES B+ ---
...
void ler_todos_participantes(const char *nome) {
    ...
    // Abertura do arquivo de localização para O(1)
    HeaderLocalizacao header_loc;
    FILE *fp_loc = abrir_arquivo_localizacao(nome_localizacao_bin, &header_loc);
    if (fp_loc == NULL) {
        fclose(fp);
